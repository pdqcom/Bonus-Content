...
#If File does not exist create it, else append hashtable to file
if(!(Test-Path -Path C:\temp)){
    New-Item -Path "C:\Temp"
}
If(!(Test-Path $file)){
    [PSCustomObject]$HT | Export-Csv -NoTypeInformation -Path $File -Force
}else{
    [PSCustomObject]$HT | Export-Csv -NoTypeInformation -Append -Path $File -Force
}

##Grab last 5 Results
$Results = Import-Csv $File | Select-Object -last $History
##Convert type data from import to usable formats
foreach($object in $Results){
    Foreach($Item in $object.psobject.properties){
        if(($item.Name -like "*drive*") -or ($item.Name -like "*Memory*") -or ($item.Name -like "*Tempurature*") -or ($item.Name -eq "CPU Utilization") -or ($item.Name -eq "Total Processes") -or ($item.Name -like "*Process ID*")){
            $item.Value = [int]$item.Value
        }elseif ($item.name -eq "Timestamp") {
            $item.Value = [datetime]$item.Value
        }
    }
}
##Build new History File
$Results | Export-Csv -NoTypeInformation -Path $File -Force
###Convert to a pscustom Object
